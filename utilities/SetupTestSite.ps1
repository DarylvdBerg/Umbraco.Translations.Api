function Create-Test-Site {
    ...
    param(
        ...
        [Parameter()]
        [string] $StarterkitVersion,

        [Parameter()]
        [bool] $InstallStarterKit
	)
    ...
    cd $Destination
    dotnet new umbraco -n $ProjectName --development-database-type LocalDB --version $CmsVersion

    cd "$Destination\$ProjectName"

    if ($InstallStarterKit) {
...
Create-Test-Site $TestSitesFolder $TestProjectName $UmbracoVersion $StarterkitVersion $InstallStarterKit
